...
During generation, a compact panel cycles six illustrative activity messages every eight seconds, displays elapsed time and keeps Cancel available. The messages are explicitly not backend stage reports. A synchronous client request lock blocks duplicate submissions. The yellow activity bar is static under reduced motion. Status messages use polite announcements; the elapsed timer is not announced every second. Focus moves to the panel on start, the result on success, and the existing error on cancellation/failure. The panel unmounts and clears its interval when the request ends; the next generation starts at zero. No AI workflow or provider settings are changed.

Loading-state verification: lint, typecheck and production build passed. The full suite completed with 67 passed and one existing skip. Desktop/mobile checks cover all six rotating messages, elapsed time, duplicate-submit blocking, success focus, timer reset, keyboard cancellation, provider failure, reduced-motion styling and axe accessibility. Progress-panel screenshots were visually reviewed.
