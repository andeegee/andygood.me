...
export default function WorkPage() {
  return <div className={styles.work}>
    ...
    <Section aria-labelledby="more-heading"><div className={styles.moreIntro}><p className={styles.eyebrow}>More selected work</p><h2 id="more-heading">Different sectors. Same need for clarity.</h2></div>
      {moreProjects.map((project, index) => <article key={project.name} className={index < 2 ? styles.support : styles.textProject} aria-labelledby={`more-${index}`}><div className={styles.copy}><header><h3 id={`more-${index}`}>{project.name}</h3><p className={styles.category}>{project.category}</p></header>{index < 2 && <p>{project.copy}</p>}</div>{index === 0 ? <div className={styles.moxieArt}><Artwork source={moxie} name="moxie-support" alt="Moxie landing-page copy concept for medspa founders, including headline, proposition and callouts" sizes="(max-width: 767px) 92vw, 440px" /></div> : index === 1 ? <div className={styles.datchArt}><Artwork source={datch} name="datch-support" alt="Supplied Datch LinkedIn campaign artwork promoting AI Asset Insights" sizes="263px" /></div> : <p>{project.copy}</p>}</article>)}
    </Section>
