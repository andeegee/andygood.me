...
export default function InsightsPage() {
  ...
  return <>
    ...
    <Container className={styles.index}>
      ...
      <div className={styles.list}>{rest.map((article) => <article className={styles.entry} key={article.slug}><div><span className={styles.eyebrow}>{article.category}</span><p className={styles.meta}>12 September 2026 · {article.readingTime}</p></div><div><h2><Link href={`/insights/${article.slug}/`}>{article.title}</Link></h2><p>{article.description}</p><Link className={styles.articleLink} href={`/insights/${article.slug}/`}>Read the article</Link></div></article>)}</div>
