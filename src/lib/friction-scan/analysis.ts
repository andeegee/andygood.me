...
Assess clarity (offer, audience, relevance), trust (specific proof, credible support, unsupported assertions), conversion (value, CTA, journey, objections and reason to act), and restrained search/AI readability (topic, useful entities, questions and structure). Return one or two specific strengths and exactly THREE prioritised friction observations with why each matters. Even for a strong page frame three proportionate opportunities with uncertainty, not invented faults. For each area give ONE strongest point, ONE main friction and ONE practical recommendation, each in one or two short sentences (roughly 20 to 35 words). Acknowledge when no strength or weakness can confidently be established. Choose one useful exact quote supporting the area as a whole where possible; retain the required quote on EVERY observation, using different quotes when needed for accurate grounding. The email displays the mainFriction quote as the featured evidence, so make its connection to the main friction explicit. Do not embed quotations or Page evidence labels in explanations. Executive summary: roughly 60 to 90 words covering overall diagnosis, main commercial friction and biggest opportunity, without listing the category findings. fixFirst: ONE decisive strategic recommendation in 30 to 45 words explaining what to change first and why it takes precedence, without claiming Andy personally reviewed the page. Return exactly THREE priorityFixes ordered by likely commercial importance. In each priorityFixes explanation put a short action heading (3 to 7 words) on the first line, then a newline and a concise explanation (15 to 25 words). These are an action recap, not repeated category analysis: do not repeat quotations or recommendations verbatim. Keep the required supporting quote for validation even where the email does not display it. Executive summary and full breakdown must agree with the immediate result. Avoid repeating the same diagnosis across categories; use distinct relevant angles without inventing faults. Prefer concise prose over a word-count target; keep the full report fields including supporting quotes below 1,200 words. Do not introduce numerical scores or an exhaustive checklist.` },
